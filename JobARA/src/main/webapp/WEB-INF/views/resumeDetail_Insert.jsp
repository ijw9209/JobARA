...
<script type="text/javascript">
   ...
   function school(){
       var con = document.getElementById("school");
       if(con.style.display=='none'){
           ...
           $('#school_select1 option:eq(0)').prop("selected", true);
           $('#school_select2 option:eq(0)').prop("selected", true);
   ...
   function career(){
       var con = document.getElementById("career");
          if(con.style.display=='none'){
              con.style.display = 'block';
          }else{
              con.style.display = 'none';
              ...
              var el = document.getElementsByClassName('career-text');
              for(var i=0; i<el.length; i++){
                 el[i].value = '';
              }
              
              var dt = document.getElementsByClassName('calendar');
              for(var i=0; i<dt.length; i++){
                 dt[i].value = '';
              }
              $("#career-select option:eq(0)").prop("selected", true);

          }
      
   }
      function certifi(){
          var con = document.getElementById("certificate");
             if(con.style.display=='none'){
                 con.style.display = 'block';
             }else{
                 con.style.display = 'none';
                 ...
                 var el = document.getElementsByClassName('certifi-text');
                 for(var i=0; i<el.length; i++){
                    el[i].value = '';
                 }
                 var dt = document.getElementsByClassName('calendar');
             ...
             function military(){
                 var con = document.getElementById("military");
                   if(con.style.display=='none'){
                       con.style.display = 'block';
                   }else{
                       con.style.display = 'none';
                       alert("병역을 삭제하시겠습니까?")
                       $("#military-select1 option:eq(0)").prop("selected", true);
                       $("#military-select2 option:eq(0)").prop("selected", true);
             ...
             function self(){
                 var con = document.getElementById("self");
                   if(con.style.display=='none'){
                       con.style.display = 'block';
                   }else{
                       con.style.display = 'none';
                       alert("자기소개서를 삭제하시겠습니까?")
                       
                       var el = document.getElementsByClassName('self-text');
                       for(var i=0; i<el.length; i++){
                          el[i].value = '';
                       }
                     // $("#self-textarea1").value='';
                      document.getElementById("self-textarea1").value='';
                      document.getElementById("self-textarea2").value='';
...
<body>
   ...
   <div id="sidebanner">
      ...
      <ul style="list-style: none;" id="sidebanner_ul">
         <li><input type="button" id="schoolbutton" value="학력"  onclick="school();"></li>

         <li><input type="button" id="careerbutton" value="경력" onclick="career();"></li>

         <li><input type="button" id="certifibutton" value="자격증" onclick="certifi();"></li>

         <li><input type="button" id="militarybutton" value="병역" onclick="military();"></li>

         <li><input type="button" id="selfbutton" value="자기소개서" onclick="self();"></li>
      </ul>
   ...
   <div id="content">
      <form action="/user/ResumeDetailInsertRes.do?member_no_seq=${seq }"   method="post" onsubmit="return hopeinterview();">
         <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
          <input type="hidden" name="member_no_seq" value="${seq }"> 
          <input type="hidden" name="member_no_seq" value="${seq }"> 
          <input type="hidden" name="member_no_seq" value="${seq }"> 
          <input type="hidden" name="member_no_seq" value="${seq }"> 
            <a><input type="text" id="resume_title"   name="resume_title" placeholder="기업에게 나를 알려줍시다."></a>

         <fieldset>
            <legend>사진등록</legend>
            <input type="hidden" id="resume_photo" name="resume_photo" value="">
            <label><img id="kfile" alt="사진추가" onclick="popUpImg();" class="imageUpload" width="103px" height="132px"/></label><br>
         </fieldset>

         <fieldset>
            <legend>인적사항</legend>
            <label><input type="text" name="member_name" value="${memberresume.member_name }"></label><br> 
            <label><input type="text" name="member_birth" class="calendar" value="${memberresume.member_birth }"></label><br> 
            ...
            <label><input type="text" name="member_email" value="${memberresume.member_email}"></label><br> 
            <label><input type="text" name="member_tel" value="${memberresume.member_tel}"></label><br>
            <label><input type="text" name="member_addr" id="resume_addr"value="${memberresume.member_addr}" ></label><br>
         </fieldset>
            ...
            <table>
               <tr>
                  <td><input type="button" value="초등학교 졸업" id="elementary">
                     ...
                     <input type="button" value="대학~대학원 이상 졸업" id="university"   onclick=""></td>
               </tr>
               <tbody id="edubody" style="display: none;">
                  <tr>
                     <th>학교명</th>
                     <td><input type="text" name="resume_final_edu" id="edutxt" style="display: none;" class="input-text"> 
                     <input type="text" placeholder="학교명 입력" class="input-text" name="resume_schoolname" id="schoolname">
                     </td>
                  </tr>
                  <tr>
                     <th>졸업 년도</th>
                     <td><input type="date" id="datepicker" name="resume_graduated_year" > 
                     <select   name="resume_graduated_state" id="school_select1">
               ...
               <tr id="major" style="display: none;">
                  <th>전공</th>
                  <td><select name="major" id="school_select2">
         ...
         <fieldset>
            <legend>경력</legend>
            <label><input type="text" name="career_ent"   placeholder="회사이름" class="career-text"></label><br> 
            <label><input type="text" name="career_dept" placeholder="부서명" class="career-text"></label><br>
            <label><input type="text" readonly="readonly" name="career_startday" class="calendar">~
                  <input type="text" readonly="readonly" name="career_endday" class="calendar"></label><br>
            <label><select name="career_in_office" id="career-select">
                  <option  value="">현재근무상태</option>
                  <option value="Y">재직중</option>
                  <option value="N">퇴사</option>
            </select></label><br> 
            <label><input type="text" name="career_position" placeholder="직급/직책" class="career-text"></label><br> 
            <label><input type="text" name="career_duty" placeholder="직무" class="career-text"></label><br>
            <label><input type="text" name="career_salary" placeholder="연봉" class="career-text"></label><br>
         </fieldset>
         </div>
         ...
         <div id="certificate" style="display: none;">
            ...
            <table>
               <tbody id="crtibody1" style="display: none">
                  ...
                  <tr>
                     <th>자격증</th>
                     <td><input type="text" placeholder="자격증명 입력" class="certifi-text" name="resume_crti_one_name" id="crtinameone"> 
                     <input type="button" value="자격증추가" id="criinsert" class="certifi-text"></td>
                  </tr>
                  <tr>
                     <th>발행처/기관</th>
                     <td><input type="text" placeholder="발행처/기관 입력"    name="resume_crti_one_issue" id="crtiissueone" class="certifi-text"></td>
                  </tr>
                  <tr>
                     <th>취득일</th>
                     <td><input type="text" placeholder="취득일 입력" name="resume_crti_one_day" id="crtidayone" class="calendar"></td>
                  </tr>
               </tbody>
               <tbody id="crtibody2" style="display: none">
                  <tr>
                     <th>자격증</th>
                     <td><input type="text" placeholder="자격증명 입력" name="resume_crti_two_name" id="crtinametwo" class="certifi-text"></td>
                  </tr>
                  <tr>
                     <th>발행처/기관</th>
                     <td><input type="text" placeholder="발행처/기관 입력" name="resume_crti_two_issue" id="crtiissuetwo" class="certifi-text"></td>
                  </tr>
                  <tr>
                     <th>취득일</th>
                     <td><input type="text" placeholder="취득일 입력" name="resume_crti_two_day" id="crtidaytwo" class="calendar"></td>
                  </tr>
               </tbody>
               <tbody id="crtibody3" style="display: none">
                  <tr>
                     ...
                     <td><input type="text" placeholder="자격증명 입력"
                        name="resume_crti_three_name" id="crtinamethree" class="certifi-text"></td>
                  </tr>
                  <tr>
                     <th>발행처/기관</th>
                     <td><input type="text" placeholder="발행처/기관 입력"
                        name="resume_crti_three_issue" id="crtiissuethree" class="certifi-text"></td>
                  </tr>
                  <tr>
                     <th>취득일</th>
                     <td><input type="text" placeholder="취득일 입력"
                        name="resume_crti_three_day" id="crtidaythree" class="calendar"></td>
                  </tr>
               </tbody>
            </table>
         </div>
         ...
         <fieldset>
            ...
            <legend>병역</legend>
            <label><select id="military" name="resume_military_chk" class="military-select">
                  <option value="">병역대상</option>
                  <option id="military_check" value="Y">군필/면제</option>
                  <option value="N">미필/복무중</option>

            </select></label><br> 
            <label class="military_detail" style="display: none;">
            <input type="text" class="calendar" name="resume_military_startday"   readonly="readonly" placeholder="입대일">~
            <input type="text" class="calendar" name="resume_military_endday" readonly="readonly" placeholder="제대일"></label><br> 
            <label><select name="resume_military_group" id="military-select1">
            ...
            <select name="resume_military_lastclass" id="military-select2">
         ...
         <fieldset>
            <legend>자기소개서</legend>
            <label><input type="text" name="resume_self_one_title" placeholder="자기소개서 제목" class="self-text"></label><br> 
            <label><textarea name="resume_self_one_content" rows="20" cols="100" placeholder="자기소개서 내용" id="self-textarea1"></textarea></label><br>
         </fieldset>
         <fieldset>
            <legend>자기소개서</legend>
            <label><input type="text" name="resume_self_two_title" placeholder="자기소개서 제목" class="self-text"></label><br> 
            <label><textarea name="resume_self_two_content" rows="20" cols="100" placeholder="자기소개서 내용" id="self-textarea2"></textarea></label><br>
         </fieldset>
         </div>
