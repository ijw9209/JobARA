			...
			<div class="col-md-7 col-sm-10 col-xs-12 center-col">
				<div class="width-100 xs-no-padding">
					<div class="width-100 padding-50px-lrb">
						...
						<div>
							<form action="registtwo" method="post" name="fr" onsubmit="return check();">
							...
							<table class="width-100 regist-table">
								...
								<tr>
									...
									<td>
										...
										<input type="text" name="member_email_addr" id="job_eamil_addr" class="form-control width-48-1 float-left" placeholder="이메일주소">
...
var getNum = RegExp(/^[0-9]{13}$/);
var idConfirm = false;
var pwConfirm = false;
var pwConfirm2 = false;
//var emailConfrim = false;
var buisNumConfirm = false;


	function check(){
		
		if(fr.job_buis_num.value == ""){
			$("#buis_span").css('color','red');
			$("#buis_span").html('사업자등록번호를 입력해주세요.');
			fr.job_buis_num.focus();
			return false;
		}
	...
	function numchk(){
		buisNumConfirm = false;
		
		if(!getNum.test($("#job_buis_num").val())){
			$("#buis_span").css('color','red');
			$("#buis_span").html('사업자등록번호 13자리를 입력해주세요.');
			$("#job_buis_num").focus();
			return false;
		}
		$("#buis_span").css('color','red');
		$("#buis_span").html('사업자등록번호완료.');
		buisNumConfirm = true;
	}
